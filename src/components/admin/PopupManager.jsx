...
import { Plus, Edit2, Trash2, Eye, EyeOff, X, Save, Upload, BarChart2 } from 'lucide-react';
...
const PopupManager = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Se attivo questo popup, disattiva gli altri
  ...
  const toggleActive = async (popup) => {
    try {
      // Se lo stiamo attivando, disattiva gli altri
      if (!popup.is_active) {
      ...
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-light text-gray-900">
            Gestione Popup ({popups.length})
          </h3>
          <p className="text-sm text-gray-600 mt-1">
            Solo un popup può essere attivo alla volta
          </p>
        </div>
        <button
          onClick={() => openModal()}
            ...
            >
              <div className="flex items-stretch">
                ...
                <div
                  ...
                  style={{ backgroundColor: popup.bg_color || '#1B7B7E' }}
                />
                ...
                <div className="flex-1 p-6">
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-4 border-t border-gray-100">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-teal-100 text-teal-700 rounded-lg hover:bg-teal-200 transition text-sm font-medium flex items-center gap-2"
                    >
                      <Edit2 size={16} />
                      Modifica
                    </button>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Es: Prenota entro il 30 giugno e ottieni il 10% di sconto!"
                  />
                </div>
                ...
                <div>
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    <span className="text-sm font-medium text-gray-700">
                      Attiva subito questo popup
                    </span>
                  </label>
                  {formData.is_active && (
                    <p className="text-xs text-amber-600 mt-1 ml-7">
                      ⚠️ Gli altri popup attivi verranno disattivati automaticamente
                    </p>
                  )}
                </div>
              </div>
                ...
                {showPreview && (
                  <div className="rounded-xl overflow-hidden border border-gray-200">
                    <div className="bg-black/50 p-8 flex items-center justify-center">
                      <div
                        className="rounded-2xl shadow-2xl max-w-sm w-full p-8 relative"
                      ...
                      >
                        ...
                        <div className="text-center" style={{ color: formData.text_color }}>
                          {formData.image_url && (
                            <img src={formData.image_url} alt="Preview" className="w-full h-36 object-cover rounded-lg mb-4" />
                          )}
                          {formData.title && (
                            <h3 className="text-2xl font-light mb-3">{formData.title}</h3>
                          )}
                          {formData.message && (
                            <p className="text-base mb-5 opacity-90">{formData.message}</p>
                          )}
                          {formData.button_text && (
                            <button className="bg-white text-teal-700 px-6 py-2 rounded-lg font-medium">
                              {formData.button_text}
                            </button>
                          )}
                        </div>
